...
$$
L(w)=\sum\limits_{i=1}^N\big||w^Tx_i-y_i|\big|^2_2
$$
展开得到：
$$
\begin{align}
...
&=\sum_{i=1}^N\big( \log(\frac{1}{\sqrt{2\pi\sigma}}) - \frac{(y_i - w^Tx_i)^2}{2\sigma^2} \big) \\
...
$$
\begin{align}
\hat{w}=\mathop{argmax}\limits_w\mathbf{p}(w|Y)&=\mathop{argmax}\limits_w\mathbf{p}(Y|w)\mathbf{p}(w)\nonumber\\
&=\mathop{argmax}\limits_w\log \mathbf{p}(Y|w)\mathbf{p}(w)\nonumber\\
&=\mathop{argmax}\limits_w \big(\log \mathbf{p}(Y|w)+\log \mathbf{p}(w) \big)\nonumber\\
&=\mathop{argmin}\limits_w \big[(y-w^Tx)^2+\frac{\sigma^2}{\sigma_0^2}w^Tw \big]
\end{align}
$$
...
$$
\begin{align}
L1&:\mathop{argmin}\limits_wL(w)+\lambda\big||w|\big|_1,\lambda\gt0\\
L2&:\mathop{argmin}\limits_wL(w)+\lambda\big||w|\big|^2_2,\lambda \gt 0
\end{align}
$$
下面对最小二乘误差分别分析这两者的区别。
...
s.t. ||w||_1\lt C
$$
我们已经看到平方误差损失函数在 $w$ 空间是一个椭球，因此上式求解就是椭球和 $\big||w|\big|_1=C$ 的切点，因此更容易相切在坐标轴上。

### L2 Ridge

$$
\begin{align}
\hat{w}=\mathop{argmin}\limits_w\big(L(w)+\lambda w^Tw\big)&\longrightarrow\frac{\partial}{\partial w}L(w)+2\lambda w=0\nonumber\\
...
可以看到，这个正则化参数和前面的 **MAP** 结果不谋而合。利用 2 范数进行正则化不仅可以是模型选择 $w$ 较小的参数，同时也避免 $ X^TX$不可逆的问题。
